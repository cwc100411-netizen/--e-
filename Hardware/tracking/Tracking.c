...
static void Tracking_ReadPacket(Tracking_PointTypeDef *Laser, Tracking_PointTypeDef Quad[], uint8_t Packet[], uint8_t Length);
...
void Tracking_Task(void)
{
	...
	uint8_t RxPacket[SERIAL_RX_PACKET_LENGTH];
	uint8_t RxLength;
	int16_t ErrorX;
	int16_t ErrorY;
	int16_t SpeedX;
	...
	if (Serial_GetTrackingPacket(RxPacket, &RxLength) == 0)
	{
	...
	Tracking_ReadPacket(&RxLaser, RxQuad, RxPacket, RxLength);
...
static void Tracking_ReadPacket(Tracking_PointTypeDef *Laser, Tracking_PointTypeDef Quad[], uint8_t Packet[], uint8_t Length)
{
	...
	{
		if (Length >= SERIAL_RX_PACKET_LENGTH)
		{
			Quad[i].X = Packet[2 + i * 2];
			Quad[i].Y = Packet[3 + i * 2];
		}
		else
		{
			/* 短包只有激光点坐标，没有四边形顶点。 */
